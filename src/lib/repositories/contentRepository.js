...
import { RESOURCES as legacyDefaultResources } from '@data/resources';
...
const defaultResources = clone(legacyDefaultResources);
...
async function readDomain(domain, fallback) {
  ...
  const fallbackItems = clone(fallback);
  ...
  try {
    ...
    if (!snap.exists()) {
      const localItems = clone(localSnapshot.items);
